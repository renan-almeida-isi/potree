...
export class Viewer extends EventDispatcher {
  constructor(domElement, args = {}) {
    ...
    try {
      ...
      this.profileTool = new ProfileTool(this);
			...
			this.annotationTool.addEventListener(
        "start_inserting_annotation",
				(e) => {
					console.log(e);
          this.annotationBeingInserted = e.annotation;
        }
			);
			
			this.annotationTool.addEventListener(
        "finished_inserting_annotation",
        () => {
          this.annotationBeingInserted = null;
        }
      );
  ...
  loadGUI(callback) {
    ...
    let tagsContainer = $("#potree_tags_container");
    tagsContainer.load(
      new URL(Potree.scriptPath + "/tags.html").href,
      () => {
        $(() => {
          let tags = new Tags(this);
          tags.init();
  ...
  updateAnnotations() {
    ...
    this.scene.annotations.traverse((annotation) => {
      ...
      const showWithinDistance = distance < 5

			const inFrustum =
        screenPos.z >= -1 && screenPos.z <= 1 && showWithinDistance && annotation.canBeDisplayed;
