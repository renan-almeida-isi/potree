...
export class Annotation extends EventDispatcher {
  ...
  set canBeDisplayed(value) {
    if (this._canBeDisplayed === value) {
      return;
    }
    this._canBeDisplayed = value;
    this.traverse((node) => {
      node._canBeDisplayed = value;
    });
  }
  ...
  async addAnnotationFromFile(file_path) {
    let response = await fetch(file_path);
    let json = await response.json();
    json.forEach((group) => {
      let { children, ...parent } = group;
      parent = new Annotation(parent);
      this.add(parent);
      if (children)
        for (let child of children) {
          parent.add(new Annotation(child));
        }
    });
  }
};
